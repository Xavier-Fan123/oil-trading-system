...
DB_PATH = Path(r"C:\Users\itg\Desktop\X\src\OilTrading.Api\oiltrading.db")
...
def parse_qty_unit(text: str) -> tuple[float, str]:
    ...
    # Keep detection robust even if source text contains non-ASCII unit markers.
    if "\u5428" in s or "\u516c\u5428" in s or " MT" in f" {upper} " or "METRIC TON" in upper:
        unit = "MT"
    elif "\u6876" in s or "BBL" in upper or "BARREL" in upper:
        unit = "BBL"
    elif "\u5347" in s or "GAL" in upper or "LITER" in upper or "LITRE" in upper:
        unit = "GAL"
    else:
        unit = "BBL"
...
def extract_best_row(req: str) -> dict[str, Any] | None:
    ...
    for row in rows.values():
        ...
        if "ITGR-" not in ext.upper():
            continue
        ctype = row_value(row, "field50978")
...
def find_purchase_by_request(req: str) -> dict[str, Any] | None:
    status, payload = sync.api_request(BASE, "GET", "purchase-contracts?page=1&pageSize=200")
    if status != 200:
        return None
...
def ensure_ref_ids(draft: dict[str, Any], partners_cache: list[dict[str, Any]], products_cache: list[dict[str, Any]]) -> tuple[str, str]:
...
def update_purchase_api(
    ...
    payload["externalContractNumber"] = draft["external_contract_number"]
...
def update_purchase_sql(conn: sqlite3.Connection, contract_id: str, ext: str, partner_id: str, product_id: str) -> None:
    conn.execute(
        """
        ...
        """,
        (
            sync.clean_text(ext).upper(),
            sync.clean_text(partner_id).upper(),
            sync.clean_text(product_id).upper(),
            "oa_x_sync_repair",
            sync.clean_text(contract_id),
        ),
    )


def main() -> None:
    if not sync.x_health(BASE):
    ...
    if not rows:
        raise RuntimeError("No repair rows extracted")

    results: list[dict[str, Any]] = []
    sql_updates: list[tuple[str, str, str, str]] = []
    ...
    for draft in rows:
        ...
        if not existing:
            results.append({"request_id": req, "external": ext, "status": "missing_in_purchase"})
            continue

        cid = sync.clean_text(str(existing.get("id") or ""))
        if not cid:
            results.append({"request_id": req, "external": ext, "status": "missing_id"})
            continue

        partner_id, product_id = ensure_ref_ids(draft, partners_cache, products_cache)
        if not partner_id or not product_id:
            results.append({"request_id": req, "external": ext, "status": "ref_create_failed"})
            continue

        st, detail = update_purchase_api(cid, draft, partner_id, product_id, trader_id)
        if st not in (200, 204):
            results.append({"request_id": req, "external": ext, "status": "api_update_failed", "detail": detail})
            continue

        sql_updates.append((cid, ext, partner_id, product_id))
        results.append({
            "request_id": req,
            "external": ext,
            "status": "updated",
    ...
    with sqlite3.connect(DB_PATH) as conn:
        for cid, ext, partner_id, product_id in sql_updates:
            update_purchase_sql(conn, cid, ext, partner_id, product_id)
        conn.commit()
