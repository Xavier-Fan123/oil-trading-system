...
export const SalesContractForm: React.FC<SalesContractFormProps> = ({
  ...
  const [laycanStart, setLaycanStart] = useState<Date | null>(new Date());
  const [laycanEnd, setLaycanEnd] = useState<Date | null>(new Date());
  ...
  useEffect(() => {
    if (existingContract) {
      setFormData(prev => ({
        ...
        productId: existingContract.productId || '',
        quantity: existingContract.quantity || 0,
        priceBenchmarkId: existingContract.priceBenchmarkId || '',
        deliveryTerms: existingContract.deliveryTerms || DeliveryTerms.FOB,
        laycanStart: new Date(existingContract.laycanStart),
        laycanEnd: new Date(existingContract.laycanEnd),
        settlementType: existingContract.settlementType || SettlementType.TT,
        pricingType: existingContract.pricingType || PricingType.Fixed,
        ...
        <Paper sx={{ p: 3 }}>
          <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
            ...
            <Box display="flex" flexDirection="column" alignItems="flex-end">
              {isEdit && existingContract?.contractNumber && (
                <Typography variant="h6" color="primary" sx={{ fontWeight: 'bold' }}>
                  Contract: {existingContract.contractNumber}
                </Typography>
              )}
              {!isEdit && (
                    ...
                    <Grid container spacing={2}>
                      <Grid item xs={12} sm={6}>
                        <FormControl fullWidth error={!!errors.customerId}>
                          ...
                          >
                            {tradingPartners?.filter(p => p.type === 1 || p.type === 2).map(partner => (
